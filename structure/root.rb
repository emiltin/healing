module Healing
  module Structure
    class Root < Cloud
      ...
      attr_accessor :remoter, :image, :instances, :armed
      ...
      def launch
        ...
        @launched = remoter.launch :num => armed.size, :key => key_name, :image => image
        @launched.each do |i|
          i.cloud=self
          i.cloud_uuid = uuid
        end
...
#          i.command "#{installer} upgrade"
      ...
      def scan
        puts_progress "Scanning" do
          @instances = remoter.instances :key => key_name, :state => :running
      ...
      def show_volumes
        volumes = []
        puts_progress("Scanning volumes") { volumes = remoter.volumes }
        if volumes.any? 
